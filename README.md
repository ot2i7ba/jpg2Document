...
1. **Clone the Repository**
   ```sh
   git clone https://github.com/ot2i7ba/jpg2Dokument.git
   cd jpg2Dokument
   ```
...
1. Prepare Your Template
   - Create a Word document (.docx) that includes the placeholder text <<jpg2Dokument>> where you want the table of images to be inserted.
   - Save this template as jpg2Dokument.docx in the same directory as the script or specify a different template using command-line arguments.
...
3. Run the Script
   ```sh
   python jpg2Dokument.py
   ```

   Optional: Use command-line arguments to customize parameters.
   ```sh
   python jpg2Dokument.py --template my_template.docx --output my_document.docx --placeholder "<<InsertImages>>" --image_width 8.0 --gap_width 0.1 --max_px 1000 --jpeg_quality 75 --doc_author "Your Name" --doc_comments "Generated by jpg2Dokument"
   ```

### Command-Line Arguments
- --template: Path to the Word template (.docx). Defaults to jpg2Dokument.docx.
- --output: Path for the output Word file. Defaults to pictures.docx.
- --placeholder: Placeholder text in the template to be replaced. Defaults to <<jpg2Dokument>>.
...
- --doc_author: Value for the document's author property. Defaults to jpg2Dokument.
- --doc_comments: Value for the document's comments property. Defaults to jpg2Dokument by ot2i7ba.
...
**Output**:
- A Word document named pictures.docx is generated, containing a table of compressed, landscape-oriented images replacing the <<jpg2Dokument>> placeholder in the template.
- Compressed images are stored temporarily in the compressed folder, which is deleted after the document is created.

## Customized Parameters
   ```sh
   python jpg2Dokument.py --template report_template.docx --output annual_report.docx --placeholder "<<AnnualImages>>" --image_width 8.0 --gap_width 0.1 --max_px 1000 --jpeg_quality 75 --doc_author "Jane Doe" --doc_comments "Annual Report Images"
   ```
...
# Releases
A compiled and packaged version of jpg2Dokument is available for download in the **[Releases](https://github.com/ot2i7ba/jpg2Dokument/releases)** section on GitHub. This version includes all necessary dependencies and can be run without requiring Python to be installed on your system.
...
# License
This project is licensed under the **[MIT license](https://github.com/ot2i7ba/jpg2Dokument/blob/main/LICENSE)**, providing users with flexibility and freedom to use and modify the software according to their needs.
...
# Conclusion
This script has been tailored to fit my specific professional needs, and while it may seem like a small tool, it has a significant impact on my workflow. jpg2Dokument is a valuable tool for automating the creation of Word documents that incorporate multiple images in a structured and visually appealing manner. By handling image compression, scaling, and placement, it simplifies the process of compiling images into reports or presentations. Greetings to my dear colleagues who avoid scripts like the plague and think that consoles and Bash are some sort of dark magic – the [compiled](https://github.com/ot2i7ba/jpg2Dokument/releases) version will spare you the console kung-fu and hopefully be a helpful tool for you as well. 😉
